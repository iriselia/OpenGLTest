...
#pragma region GLFW/GLEW Initialization
	// Initialise GLFW
	if (!glfwInit())
	{
	...
	window = glfwCreateWindow(1024, 768, "Computer Animation Project 1", NULL, NULL);
	if (window == NULL)
	{
...
#pragma region Resource Loading
	// Read spline file
	pAnimFactory = new AnimationFactory;
	pAnimFactory->readFile("spline1.txt");
	auto pAnim = pAnimFactory->createCatmullRomAnim3D();
	if (!pAnim)
	{
		printf("Error reading spline file.\n");
		getchar();
	}
	pAnim->Start();
	...
	bool res = loadAssImp("something.obj", indices, vertices, uvs, normals);
	//Revent¨®n
...
#pragma endregion
	auto pClock = new Clock;
	static float lastTime = 0;
	static float tElapsed = 0;
	do
	{
		lastTime = tElapsed;
		tElapsed = (float)pClock->split() / 1000;
		float dt = tElapsed - lastTime;
		delta_t++;
		...
		glm::vec3 movement = pAnim->Update(dt);
		movement *= glm::vec3(1, 1, 1);
		ModelMatrix = glm::translate(ModelMatrix, movement);
		//ModelMatrix = glm::translate(ModelMatrix, dt * glm::vec3(100.0f, 0, 0));
