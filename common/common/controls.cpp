...
void computeMatricesFromInputs(){
	...
	if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
	{
		...
		if (lastMousePos == glm::vec2(-1, -1))
		{
			...
			// Just clicked, do nothing but set initial values
		...
		else
		{
			...
			horizontalAngle += mouseSpeed * float(mouseStartPos.x - xpos);
			verticalAngle += mouseSpeed * float(mouseStartPos.y - ypos);
	...
	{
		// proceed only if left button is just released
		if (lastMousePos != glm::vec2(-1, -1))
		{
